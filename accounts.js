  ...
  const SEED_ACCOUNTS = [
    {
      ...
      qbr: {
        ...
        prompts: {
          health: [
            'Are we confident in the 91 score or are there gaps in our data we should sanity-check before presenting?',
            'Is there anything not captured in the health score that the team should know going into this call?',
            'Who on our side is responsible for keeping this account green through Q2?',
          ],
          adoption: [
            'Do we know why Sales is trailing at 79%? Is it a product fit issue, a training gap, or something else?',
            'Should we proactively offer a Sales team enablement session before the next check-in?',
            'Are we tracking adoption trending or just point-in-time? Do we have a story to tell about momentum?',
          ],
          risks: [
            'Even with no active risks, what\'s the most likely thing to go wrong with this account in the next 90 days?',
            'Is the UK expansion a real opportunity we can resource, or are we getting ahead of ourselves?',
            'Are there any internal signals — support volume, login frequency — that don\'t match the green status?',
          ],
          renewal: [
            'Who owns the renewal conversation internally — is it the TAM, AE, or both?',
            'Do we have a target ARR for the renewal, and does the UK expansion factor into that number?',
            'What\'s our walk-in position if they push back on expansion pricing?',
          ],
          nextSteps: [
            'Who is sending the follow-up after this call and within what timeframe?',
            'Do we have a Q2 scoping call on the calendar or are we leaving this meeting to set that up?',
            'What does success look like for this account 6 months from now and are we aligned internally on that?',
          ],
        },
      },
      qbrClient: {
...

    {
      ...
      qbr: {
        ...
        prompts: {
          health: [
            'Are we fully aligned internally on why the score dropped to 63 and what caused it?',
            'Is engineering on track to resolve ticket #1042 before the client call, or do we need to set expectations?',
            'Who owns communicating the ticket resolution status to the client after this meeting?',
          ],
          adoption: [
            'Do we actually know what caused the Finance drop — is it the ticket, or is there something else going on?',
            'Should we be doing a proactive Finance team outreach before the QBR to gather context?',
            'Operations is also slipping at 72% — are we planning to address that in this call or hold it for the next one?',
          ],
          risks: [
            'Are both risks (ticket and adoption) being owned by the same person or split? Is that clear?',
            'What happens if the ticket isn\'t resolved by the time we sit down with the client?',
            'Is the 11-day touch gap a process failure or was there a reason? How do we prevent it going forward?',
          ],
          renewal: [
            'What is our internal renewal goal — full renewal at same ARR, expansion, or are we at risk of downsell?',
            'Has the cancellation deadline passing been flagged to leadership internally?',
            'What\'s our plan B if they don\'t want to renew? Do we have an off-ramp conversation prepared?',
          ],
          nextSteps: [
            'Who is responsible for the two-week ticket resolution follow-up and how will we track it?',
            'Who needs to be on the renewal call from our side — just TAM or do we need an AE or CSM lead?',
            'Are we aligned on what we\'re asking the client to commit to before we leave this meeting?',
          ],
        },
      },
      qbrClient: {
...

    {
      ...
      qbr: {
        ...
        prompts: {
          health: [
            'Do we know enough to walk into this meeting, or are there things we should have found out first?',
            'Is anyone on our side accountable for letting this account get to a 31 without an earlier intervention?',
            'What is our honest internal assessment — is this account saveable, and do we want to save it?',
          ],
          adoption: [
            'Do we have any idea why Scheduling dropped 22 points? Is it a product issue, a people issue, or something we did?',
            'Engineering at 12% is nearly zero — should we be asking our implementation team if something was misconfigured?',
            'Are we prepared to present this data or should we lead with questions before showing any slides?',
          ],
          risks: [
            'Has anyone tried to reach Marcus Webb\'s replacement directly before this meeting? If not, why not?',
            'What\'s our escalation path if this meeting goes poorly — who internally needs to be looped in today?',
            'Are we walking in with a re-engagement plan ready or are we genuinely just here to listen?',
          ],
          renewal: [
            'Internally, are we trying to save this renewal or are we doing due diligence before an expected churn?',
            'If they want to renew, do we have capacity and a plan to actually re-engage them properly?',
            'What are we willing to offer — extended trial, concession, re-onboarding support — and has that been approved?',
          ],
          nextSteps: [
            'What is the single most important thing we need to leave this meeting with — a name, a call, a decision?',
            'Who is writing the follow-up and within what timeframe — today, tomorrow, this week?',
            'If they say they\'re not renewing, what does our internal handoff process look like?',
          ],
        },
      },
      qbrClient: {
